                    ...
                    <div class="w-full p-6">
                        <p class="text-gray-700">
                            @if (!auth()->user()->email_verified_at)
                                ...
                                <form id="resend-verification-email-form" action="{{ route('resend_verification_email') }}" method="POST" class="mt-4">
                                    {{ csrf_field() }}
                            ...
                            @else 
                                @if (auth()->user()->tokens->count())
                                    ...
                                    <div class="flex mt-4">
                                        <form id="update-token" action="{{ route('tokens.update') }}" method="POST" class="">
                                            @csrf
                                            @method('patch')
                                            <button type="submit" class="inline-block px-4 py-2 text-base font-bold leading-normal text-center text-gray-100 no-underline whitespace-no-wrap align-middle bg-green-500 border rounded select-none hover:bg-green-700"
                                        ...
                                        <form id="update-token" action="{{ route('tokens.destroy') }}" method="POST" class="ml-2 ">
                                            @csrf
                                            @method('delete')
                                            <button type="submit" class="inline-block px-4 py-2 text-base font-bold leading-normal text-center text-gray-100 no-underline whitespace-no-wrap align-middle bg-red-500 border rounded select-none hover:bg-red-700"
                                ...
                                <form id="create-token" action="{{ route('tokens.store') }}" method="POST" class="mt-4">
                                    ...
                                    <div class="flex flex-wrap mb-6">
                                        ...
                                        <input id="project" type="text" class="form-input w-full @error('project') border-red-500 @enderror" name="project" value="{{ old('project') }}" required>
                                    ...
                                    <div class="flex flex-wrap mb-6">
                                        ...
                                        <input id="url" type="text" class="form-input w-full @error('url') border-red-500 @enderror" name="url" value="{{ old('url') }}" required >
                                    ...
                                    <div class="flex flex-wrap mb-6">
                                        ...
                                        <textarea id="description" class="form-textarea w-full @error('description') border-red-500 @enderror" name="description" value="{{ old('description') }}" rows="3" autocomplete="off"></textarea>
                                        @error('description')
                                            <p class="mt-4 text-xs italic text-red-500">
                                                {{ $message }}
                                            </p>
                                    ...
                                    <div class="flex flex-col flex-wrap mb-6">
                                        <div>
                                            <label class="@error('accept') text-red-500 @enderror">
                                                ...
                                                <span class="ml-2">I accept the <a target='_blank' href="{{route('terms_of_service')}}" class="text-blue-700">Terms of Service</a> of BraveBat</span>
                                            </label>
                                        </div>
                                        @error('accept')
                                            <p class="mt-4 text-xs italic text-red-500">
